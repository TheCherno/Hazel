...
namespace Hazel {
	...
	namespace Utils {
		...
		static void AttachColorTexture(uint32_t fbo, uint32_t id, int samples, GLenum format, uint32_t width, uint32_t height, int index)
		{
	...
	void OpenGLFramebuffer::Invalidate()
	{
			...
			for (size_t i = 0; i < m_ColorAttachments.size(); i++)
			{
				...
				switch (m_ColorAttachmentSpecifications[i].TextureFormat)
				{
					case FramebufferTextureFormat::RGBA8:
						Utils::AttachColorTexture(m_RendererID, m_ColorAttachments[i], m_Specification.Samples, GL_RGBA8, m_Specification.Width, m_Specification.Height, i);
						break;
				}
			}
		}
		...
		HZ_CORE_ASSERT(glCheckNamedFramebufferStatus(m_RendererID, GL_FRAMEBUFFER) == GL_FRAMEBUFFER_COMPLETE, "Framebuffer is incomplete!");
